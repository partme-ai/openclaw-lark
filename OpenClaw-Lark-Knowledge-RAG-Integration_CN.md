# OpenClaw Lark 知识库 RAG 集成指南

## 概述

本文档描述如何在 **OpenClaw Lark （飞书） 渠道插件** 中集成 `@partme.ai/openclaw-knowledge` 知识库 RAG 模块。
...
- OpenClaw Lark 插件版本 >= 0.1.0
- 配置文件 `openclaw.plugin.json` 中已有 `channels.lark.knowledge` 配置段
...
```bash
# 在工作空间根目录
pnpm add @partme.ai/openclaw-knowledge --filter @mocrane/lark
```

### 2. 注册知识库模块

在 `openclaw-lark/src/index.ts` 中：
...
async onRegister(config, { registerHook, registerTool }) {
  const knowledge = registerKnowledgeHooks(config, {
    configPath: "channels.lark.knowledge",
  });
...
在 `openclaw.plugin.json` 的 `channels.lark` 下添加：
...
- 建议在 Agent Prompt 中引导 AI 明确区分"添加"和"查询"场景
