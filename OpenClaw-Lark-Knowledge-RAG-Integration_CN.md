# OpenClaw-Lark 知识库 RAG 集成文档

> **将 `@partme.ai/openclaw-knowledge` 独立知识库引擎集成到 `openclaw-lark` 渠道插件中**，为飞书（Lark）AI 机器人提供完整的 RAG 能力：文档自动索引、语义检索、多租户隔离、AI 主动 CRUD 操作。
...
### 1.2 与 openclaw-lark 的关系

`openclaw-lark` 是一个**渠道插件**（Channel Plugin），负责飞书的消息收发与事件回调。
...
# 输出示例
# openclaw-lark@2026.3.24-beta /path/to/openclaw-lark
...
export function onRegister(api: PluginApi) {
  ...
  // 第二个参数 "channels.lark.knowledge" 指定配置读取路径
...
| 要点 | 说明 |
|------|------|
| **Tool 名称** | 注册为 `knowledge_*`（**不是** `lark_knowledge_*`），保持与 `@partme.ai/openclaw-knowledge` 命名一致 |
| **配置路径** | `registerKnowledgeHooks(api, 'channels.lark.knowledge')` 告诉引擎从 `channels.lark.knowledge` 读取配置 |
| **纯加法** | 不修改原有 Bot/Agent 消息流的任何逻辑 |
| **错误安全** | Hook 内部 try/catch 兜底，失败时静默返回 `undefined`，不影响主流程 |
| **通道过滤** | Hook 内部按 `ctx.channelId === 'lark'` 过滤，不影响其他通道 |
...
知识库 RAG 的所有配置项位于 **`channels.lark.knowledge.*`** 路径下。
...
不同的飞书租户（account）可以拥有独立的知识库，配置继承全局值，account 级进行深度覆盖。
...
> **注意**：Tool 名称为 `knowledge_*`（而非 `lark_knowledge_*`），与 `@partme.ai/openclaw-knowledge` 的导出命名保持一致，便于在跨渠道插件中复用同一套知识库 Tool。
...
# 预期输出
# [Knowledge] registerKnowledgeHooks: configPath=channels.lark.knowledge
...
```bash
# 检查配置是否正确读取
openclaw config get channels.lark.knowledge.enabled
# → true

openclaw config get channels.lark.knowledge.embedding.model
# → text-embedding-3-small
```

### 6.3 端到端验证

1. 向飞书 Bot 发送一条包含关键信息的消息，如"记住这个：我的工号是 2024001"
...
| 查看配置 | 检查知识库配置项 | `openclaw config get channels.lark.knowledge` |
...
# 2. 确认 embedding 配置正确
openclaw config get channels.lark.knowledge.embedding
...
| 知识库 RAG 架构设计 | [架构文档](./OpenClaw-Lark-Knowledge-RAG-Architecture_CN.md) |
| 安装与配置指南 | [使用指南](./OpenClaw-Lark-Knowledge-RAG-Guide_CN.md) |
